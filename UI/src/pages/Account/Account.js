...
const Account = () => {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 max-w-7xl mx-auto">
      ...
      {userInfo?.email && (
        <>
          <p className="text-2xl font-bold">Hello {userInfo?.firstName}</p>
          <p className="mb-6 text-gray-600">Welcome to your account</p>

          <div className="flex flex-col md:flex-row gap-4">
            <ul className="w-full md:w-1/4 flex flex-col gap-4 text-sm font-medium text-gray-500">
              {[
                { to: "/account-details/profile", label: "Profile", icon: (
                    <svg className="w-4 h-4 me-2 text-white" fill="currentColor" viewBox="0 0 20 20">
                      <path d="M10 0a10 10 0 1 0 10 10A10.011 10.011 0 0 0 10 0Zm0 5a3 3 0 1 1 0 6 3 3 0 0 1 0-6Zm0 13a8.949 8.949 0 0 1-4.951-1.488A3.987 3.987 0 0 1 9 13h2a3.987 3.987 0 0 1 3.951 3.512A8.949 8.949 0 0 1 10 18Z" />
                    </svg>
                  )
                },
                { to: "/account-details/orders", label: "Orders", icon: (
                    <svg className="w-4 h-4 me-2 text-white" fill="currentColor" viewBox="0 0 20 20">
                ...
                { to: "/account-details/settings", label: "Settings", icon: (
                    <svg className="w-4 h-4 me-2 text-white" fill="currentColor" viewBox="0 0 20 20">
                  ...
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      [
                        ...
                        "inline-flex items-center px-4 py-3 text-white rounded-lg w-full transition-colors",
                      ].join(" ")
                    }
                  >
                    {icon}
